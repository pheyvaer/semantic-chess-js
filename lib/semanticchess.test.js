...
<http://example.org/myGame> <http://purl.org/NET/rdfchess/ontology/hasFirstHalfMove> <http://example.org/myfile#1>.
  ...
  describe('promotion', function() {
    ...
    it('valid moves', async function() {
		...
		semanticChess.chess = new chess.Chess('8/4P3/8/8/8/8/K6k/8 w - - 0 1');
		assert.ok(semanticChess.isPromotion('e7','e8'));
		//Test in one side of the board (white pawn)
		semanticChess.chess = new chess.Chess('8/P7/8/8/8/8/K6k/8 w - - 0 1');
		assert.ok(semanticChess.isPromotion('a7','a8'));
		//Test in one side of the board (black pawn)
		semanticChess.chess = new chess.Chess('8/7k/8/8/8/8/K6p/8 b - - 0 1');
		assert.ok(semanticChess.isPromotion('h2','h1'));
		//If the pawn is capturing another piece
		semanticChess.chess = new chess.Chess('3n4/4PP1k/7r/8/8/8/K3p3/8 w - - 0 1');
  ...
  it('invalid moves', async function() {
		...
		semanticChess.chess = new chess.Chess('8/4PP1k/8/7r/q7/8/K7/2n4Q w - - 0 1');
		assert.ok(!semanticChess.isPromotion('e7','e8'));
		//If we do an absurd movement
		semanticChess.chess = new chess.Chess('8/5P1k/8/7r/2n1P3/8/K7/7Q w - - 0 1');
		assert.ok(!semanticChess.isPromotion('e4','e8'));
		semanticChess.chess = new chess.Chess('4n3/4PP1k/8/K6r/8/2n5/4p3/7Q w - - 0 1');
		assert.ok(!semanticChess.isPromotion('e7','e8'));
		//If we do another random valid movement
		semanticChess.chess = new chess.Chess('8/4PP1k/8/7r/2n5/8/K3p3/7Q b - - 0 1');
		assert.ok(!semanticChess.isPromotion('h5','h1'));
		semanticChess.chess = new chess.Chess('8/4PP1k/8/K6r/8/2n5/4p3/7Q w - - 0 1');
		assert.ok(!semanticChess.isPromotion('h1','a8'));
		//If is not your turn
		semanticChess.chess = new chess.Chess('8/4PP1k/8/7r/2n5/8/K3p3/7Q b - - 0 1');
		assert.ok(!semanticChess.isPromotion('e7','e8'));
		//If is not a pawn
		semanticChess.chess = new chess.Chess('8/4PP1k/8/K6r/8/2n5/4p3/7Q b - - 0 1');
