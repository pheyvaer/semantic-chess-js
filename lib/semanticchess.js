...
class SemanticChess {
  ...
  isPromotion(source, target){
    ...
    var move = this.chess.move(moveToTest);
    if (move === null)
      return false;
    else
      this.chess.undo();
    
    var rankSource = source.substring(2,1);
    var rankTarget = target.substring(2,1);
    var piece = this.chess.get(source).type;

    if (piece === 'p' &&
      ((rankSource === '7' && rankTarget === '8') || (rankSource === '2' && rankTarget === '1'))){
        return true;
    }
    return false;
  }
